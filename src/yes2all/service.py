...
def read_installed_args() -> dict | None:
    """Parse the installed launchd plist and return {ports, interval, sweep_tabs}.
    ...
    try:
        ...
        if not ports:
            ports = [9222]
        return {"ports": ports, "interval": interval, "sweep_tabs": sweep_tabs}
    except Exception:
        return None


def launchd_plist(ports: list[int], interval: float, log_dir: Path, sweep_tabs: bool = True) -> str:
...
def launchd_install(ports: list[int], interval: float, sweep_tabs: bool = True) -> None:
    plist_path = launchd_plist_path()
    log_dir = Path.home() / "Library" / "Logs" / "yes2all"
    plist_path.parent.mkdir(parents=True, exist_ok=True)
    plist_path.write_text(launchd_plist(ports, interval, log_dir, sweep_tabs=sweep_tabs))
...
def systemd_unit(ports: list[int], interval: float, sweep_tabs: bool = True) -> str:
    exe = _yes2all_executable()
    sweep_flag = "--sweep-tabs" if sweep_tabs else "--no-sweep-tabs"
...
[Service]
ExecStart={exe} watch {port_args} --interval {interval} {sweep_flag}
Restart=always
RestartSec=2
...
def systemd_install(ports: list[int], interval: float, sweep_tabs: bool = True) -> None:
    unit_path = systemd_unit_path()
    unit_path.parent.mkdir(parents=True, exist_ok=True)
    unit_path.write_text(systemd_unit(ports, interval, sweep_tabs=sweep_tabs))
...
def install(ports: list[int], interval: float, sweep_tabs: bool = True) -> None:
    sysname = platform.system()
    if sysname == "Darwin":
        launchd_install(ports, interval, sweep_tabs=sweep_tabs)
    elif sysname == "Linux":
        systemd_install(ports, interval, sweep_tabs=sweep_tabs)
    else:
        raise RuntimeError(f"Unsupported platform for service install: {sysname}. "
